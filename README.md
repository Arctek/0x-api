...
| `ORDERBOOK_MODE`                       | Required. Default for dev: `MESH`                               | The mode the swap service should run in. Either `MESH` or `SRA`.                                                                                                           |
| `MESH_WEBSOCKET_URI`                   | Optional. No default                                            | The URL pointing to the 0x Mesh node. A default node is spun up in `docker-compose up`                                                                                                                    |
...
| `SRA_WEBSOCKET_URI`                    | Optional. No default                                            | The URL pointing to the 0x SRA Websocket. Optional for `ORDERBOOK_MODE` running as `SRA`.                                                                                                                    |
| `SRA_HTTP_URI`                         | Optional. No default                                            | The URL pointing to the 0x SRA HTTP endpoint. Required for `ORDERBOOK_MODE` running as `SRA`.                                                                                                                    |
...
| `DATABASE_URI`                         | Required. Default for dev: `postgresql://api:api@localhost/api` | A URI of a running postgres or mysql instance. By default, the API will create all necessary tables. A default instance is spun up in `docker-compose up`                                                          |
